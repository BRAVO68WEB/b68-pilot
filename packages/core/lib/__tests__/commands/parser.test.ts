...
describe('parseBotCommand', () => {
    ...
    test('parses close command', () => {
        const result = parseBotCommand('@b68-pilot close', appSlug)
        expect(result).not.toBeNull()
        expect(result!.command).toBe('close')
    })

    test('parses approve command', () => {
        const result = parseBotCommand('@b68-pilot approve', appSlug)
        expect(result).not.toBeNull()
        expect(result!.command).toBe('approve')
    })

    test('parses merge command', () => {
        const result = parseBotCommand('@b68-pilot merge', appSlug)
        expect(result).not.toBeNull()
        expect(result!.command).toBe('merge')
    })

    test('parses summarize command', () => {
        const result = parseBotCommand('@b68-pilot summarize', appSlug)
        expect(result).not.toBeNull()
        expect(result!.command).toBe('summarize')
    })

    test('parses summary alias', () => {
        const result = parseBotCommand('@b68-pilot summary', appSlug)
        expect(result).not.toBeNull()
        expect(result!.command).toBe('summarize')
    })

    test('parses status command', () => {
        const result = parseBotCommand('@b68-pilot status', appSlug)
        expect(result).not.toBeNull()
        expect(result!.command).toBe('status')
    })

    test('parses legacy close issue command', () => {
        const result = parseBotCommand('@b68-pilot close issue', appSlug)
        expect(result).not.toBeNull()
        expect(result!.command).toBe('close')
    })

    test('parses legacy close pr command', () => {
        const result = parseBotCommand('@b68-pilot close pr', appSlug)
        expect(result).not.toBeNull()
        expect(result!.command).toBe('close')
    })

    test('parses legacy merge pr command', () => {
        const result = parseBotCommand('@b68-pilot merge pr', appSlug)
        expect(result).not.toBeNull()
        expect(result!.command).toBe('merge')
    })

    test('parses legacy approve pr command', () => {
        const result = parseBotCommand('@b68-pilot approve pr', appSlug)
        expect(result).not.toBeNull()
        expect(result!.command).toBe('approve')
    })

    test('ignores unknown commands', () => {
        expect(parseBotCommand('@b68-pilot deploy', appSlug)).toBeNull()
    })
